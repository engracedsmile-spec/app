...
export const BookingLocationPicker = ({ onNext, initialData, routes, terminals, isProcessing }: BookingLocationPickerProps) => {
    ...
    const availableToTerminals = routes.filter(r => r.originTerminalId === fromTerminalId).map(r => terminals.find(t => t.id === r.destinationTerminalId)).filter(Boolean) as Terminal[];
