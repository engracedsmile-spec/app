...
const FundWalletDialogContent = ({ setOpen }: { setOpen: (open: boolean) => void }) => {
    ...
    const initializePayment = usePaystackPayment({
        ...
        publicKey: paymentSettings?.paystackLivePublicKey,
    });
    
    const onSubmit = (data: z.infer<typeof fundWalletSchema>) => {
        if (!paymentSettings?.paystackLivePublicKey) {
            ...
            <DrawerBody>
                <Form {...form}>
                    <form onSubmit={form.handleSubmit(onSubmit)} id="fund-wallet-form" className="space-y-4 pt-4">
                        ...
                        />
                        <Button type="submit" className="w-full h-12 text-lg" disabled={isLoading || !paymentSettings?.paystackLivePublicKey}>
